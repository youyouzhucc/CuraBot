(function () {
  ...
  function renderRefs(refIds) {
    ...
    const items = refIds
      ...
      .map(
        (r) =>
          `<li><strong>${escapeHtml(r.title)}</strong> — <span class="muted">${escapeHtml(
            r.role || ""
          )}</span></li>`
      );
    if (!items.length) return "";
    return `<div class="refs"><h4>知识库引用（书目/指南元数据）</h4><ul>${items.join("")}</ul></div>`;
  }
  ...
  function renderStep() {
    ...
    if (!step) {
      host.innerHTML = `<p class="error">步骤缺失：${escapeHtml(state.stepId)}</p>`;
      return;
    }
  ...
  function renderOutcome() {
    ...
    host.innerHTML = `
      <div class="outcome ${levelClass}">
        ...
        ${renderRefs(o.refIds)}
        <div class="row">
          <button type="button" class="btn secondary" id="btnRestartFlow">重来本流程</button>
          <button type="button" class="btn" id="btnBackMenu">返回菜单</button>
        </div>
      </div>
    `;
  ...
  function renderReferencesPage() {
    ...
    host.innerHTML = Object.keys(byCat)
      .sort()
      .map((cat) => {
  ...
  function renderBodyMap() {
    ...
    host.innerHTML = `
      <p class="muted">用于统一“身体部位”命名，便于与解剖图谱对照；不用于自动诊断。</p>
      <div class="region-grid">
        ${regions
          .map(
            (r) => `
          <div class="card region" data-region="${escapeHtml(r.id)}">
