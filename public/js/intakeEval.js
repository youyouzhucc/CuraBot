...
(function (global) {
  ...
  const CRITICAL = {
    cat_uro_er: {
      ...
      detailExplain:
        "猫**尿很少、尿不出**，还疼、肚子不舒服或吐，首先要想到**尿道堵了**这类急事——完全梗阻若不及时处理，可在数日内继发全身恶化（如氮质血症等），**公猫更常见，但母猫也不能大意**。\n\n" +
        "是不是堵、怎么治，**必须医生摸肚子、做检查才能定**，这里不能代替诊断。",
      homeCare:
        "• 别硬灌水、别自己塞药或导尿。\n• 路上别太折腾，尽快送到能看猫急诊的医院。\n• 记一下：最后一次尿出来大概几点、尿团有多小、吐了几回。",
      vetNeed:
        "**建议**：立即赴急诊；向接诊人员说明「最后一次排尿时间、是否呕吐、是否完全无尿」。",
      refIds: ["mvm-uo", "feline-patient", "isfm", "silverstein-ecc", "mvm"],
    },
    dog_gdv_er: {
      level: "emergency",
  ...
  /** 犬采集：跛行时补充运动安全提示（非诊断）。 */
  function mergeDogLameHint(f, detailExplain, homeCare) {
    let d = detailExplain;
    let h = homeCare;
    if (f.has("d_lame_yes")) {
      d +=
        "\n\n**步态线索（你选了跛行）**：持续跛行可能来自软组织扭伤、十字韧带、髌骨脱位、髋关节发育不良、指间感染或趾甲断裂等，**在查清前不宜强行加练或长跑**。\n";
      h += "• **在家**：先减少跑跳与光滑地面急转；记录哪条腿、从何时开始、有无肿热或外伤，就诊时给医生看。\n";
    }
    return { detailExplain: d, homeCare: h };
  }

  /** 猫采集：根据体重/体况选项追加科普与沟通要点（非诊断）。 */
  function mergeCatWeightRefinement(f, detailExplain, homeCare, refIds) {
    let d = detailExplain;
    let h = homeCare;
    if (f.has("wt_gain")) {
      d +=
        "\n\n**体重线索（你选了近期发胖）**：超重与关节、泌尿、内分泌等风险相关；**每日目标热量、安全运动形式与减重速度**须兽医根据体检与化验制定。可参考 AAHA 体重管理指南中「饮食与活动结合」的框架与医生沟通，不要自行长期断粮或短期内猛增运动量。\n";
      h +=
        "• **体重与活动**：每周固定条件称一次；以短时段、多回合的玩耍与漏食觅食为主；就诊时可问目标体况（BCS）与可接受的每周减重幅度。\n";
      refIds.add("aaha-weight-url");
      refIds.add("isfm-icat-url");
    } else if (f.has("wt_loss")) {
      d +=
        "\n\n**体重线索（你选了近期明显下降）**：须结合食量、饮水、尿量、吐拉与化验排查，可能与口腔疼痛、甲状腺、肾病、肿瘤或吸收不良等有关，不宜当作「正常」长期观察。\n";
      h +=
        "• **记录**：每天吃多少、喝多少、尿团大小、吐拉；若伴精神差或废食，优先门诊。\n";
    }
    return { detailExplain: d, homeCare: h };
  }
  ...
  function evaluateCat(flags) {
    ...
    if (
      ...
      const w = mergeCatWeightRefinement(f, detailExplain, homeCare, refIds);
      return finalize(
        "emergency",
        "更像要马上去医院（急诊）",
        w.detailExplain,
    ...
    const wMon = mergeCatWeightRefinement(f, detailExplain, homeCare, refIds);
    return finalize(
      "monitor",
      "先在家多盯几天，但别掉以轻心",
      wMon.detailExplain,
  ...
  function evaluateDog(flags) {
    ...
    if (f.has("d_toxin_yes")) {
      return finalize(
        ...
        new Set(["mvm-toxic-intro", "toxicology", "five-min-toxic", "silverstein-ecc", "mvm"]),
        f,
        "dog"
      );
    }
    ...
    if (
      ...
      const wl = mergeDogLameHint(f, detailExplain, homeCare);
      return finalize(
        "urgent",
        "吐、拉、不吃、瘸、蔫：建议尽快约门诊",
        wl.detailExplain,
    ...
    const wm = mergeDogLameHint(f, detailExplain, homeCare);
    return finalize(
      "monitor",
      "先在家盯紧点，大狗也要防「胀肚子急症」",
      wm.detailExplain,
